...
reading_list <- read.csv("Documents/GitHub/fluctuating-meta-analysis/april 2023/savedrecs_may_2023.csv")
reading_list <- reading_list %>% rename("ABSTRACT" = "Abstract",
                                        "TITLE" = "Article.Title")
effort_distribute(reading_list, initialize = TRUE, reviewers = "katie", save_split = TRUE)

#take new effort file (effort_katie.csv) and merge it with effort_katie3.csv (previously categorized papers)
categorized <- read.csv("effort_katie3.csv")
new_list <- read.csv("effort_katie.csv")

all_refs <- rbind(new_list,categorized) %>% filter(!duplicated(DOI))
write.csv(all_refs,"effort_katie3.csv")
